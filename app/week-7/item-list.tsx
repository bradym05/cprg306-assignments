        ...
        <div>
            ...
            <div className="w-full flex flex-col md:flex-row items-center ">
                ...
                <button
                    ...
                    className={[
                        `p-3 h-20 m-4 rounded-2xl shadow-xs font-semibold hover:text-gray-800 hover:bg-white text-gray-600`,
            ...
            <div className="w-full flex flex-col items-center"> 
                <ol className="cursor:pointer w-full">
                    {sortBy != null && getSortedItems().map((itemProps, i) => (
                        <Item key={i} {...itemProps} onSelect={() => onItemSelect(itemProps)} />
                    ))}
                    {sortBy == null && categoryKeys.map((category, i) => (
                        <div
                            key={category}
                            className="cursor:pointer bg-gray-300 rounded-2xl shadow-xs m-5 p-4"
                        >
                            <h2 className="p-6 m-4 bg-white rounded-2xl shadow-xs text-black font-bold">
                                {category}
                            </h2>
                            <div className="cursor:pointer w-full flex flex-row items-center">
                                <ol className="cursor:pointer w-full">
                                    {categoryRecord[category].map((itemProps: typeof Item.prototype, i: number) => (
                                        <Item key={i} {...itemProps} onSelect={() => onItemSelect(itemProps)}/>
                                    ))}
                                </ol>
                            </div>
